...
# If Disposition is "Approve in Full" and Claim.Amount or Close.Amount is not# If Disposition is "Approve in Full" and Claim.Amount or Close.Amount is not ""
...
apply(dat, 1, function(x){
    ...
    if (x["Disposition"] == "Approve in Full") {
      ...
      else if (x["Claim.Amount"] != "" & x["Close.Amount"] == ""){
        x["Close.Amount"] = x["Claim.Amount"]
      }
  }
})

# Consolidate Status and Disposition values

# Make sure all variants of Disposition value "Settled" are the same value

# If row has a Claim.Amount with value "" or "-" and a Disposition of   has value of "", "-", or "Other:
keepRows = dat$Item != "" & dat$Item != "-" & dat$Item != "Other"
sum(keepRows)
dat = dat[keepRows,]
...
### Dealing with Approved in Full in Status
```{r, echo=TRUE}
levels(dat$Status)[levels(dat$Status) %in% c("Approve in Full",'Approved')] <- 'Approved'

dat[dat$Status == "Approved",]$Close.Amount  = dat[dat$Status == "Approved",]$Claim.Amount

dat[dat$Status == "Approved",]$Disposition  = "Approve in Full"

#THIS DOESNT WORK RIGHT HERE FOR SOME REASON
levels(dat$Disposition) = c(levels(dat$Disposition), "Canceled")
dat[dat$Status == "Canceled",]$Disposition  = "Canceled"

 dat[dat$Status == "Canceled" | dat$Status == "Denied",]$Close.Amount  = 0 
...
```{r}
#test stuff:
table(dat$Status, dat$Disposition)
dat[dat$Status == "Approved" & dat$Disposition == "Approve in Full", c("Disposition", "Close.Amount", "Claim.Amount")]
```
